...
def test_promotion_job(
    ...
    assert out["model_version"].version == out["version"], "Model version number should be the same!"
    assert out["model_version"].run_id == model_version.run_id, "Model version run id should be the same!"
